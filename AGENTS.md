...
- **`bench_invoke` 的每一次反射呼叫拆開來看，貴的是配置不是反射**：20M 次呼叫的 invoke 迴圈實測 540／612／601 ms
  （約 27 ns/次），而其中約 23 ns 是 `Object` API 逼出來的**兩次配置**——引數裝箱（呼叫端）與結果裝箱（執行期
  invoker thunk 裡）——以 `ty_alloc(16)` 實測 11.5 ns 計算，20M × 2 × 11.5 ns ≈ 460 ms，與整段數字相符。剩下的
  4～8 ns 才是機制：一次方法表走訪、`call_invoker` 的 setjmp catch frame（獨立量測 5.63／5.91／5.70 ns/次，
  `gcc -O2`、20M 次）、拆箱與欄位存取。**Java 一樣要付那兩次配置**（它的 `invoke` 也收 `Object[]`），差別在配置器：
  Java 的 TLAB 是指標碰撞，我們走的是 free list 慢路徑。所以「反射呼叫比直接呼叫慢約 17 倍」是對的，但把它讀成
  「反射很貴」是錯的——要縮這個差距，該動的是**配置快速路徑**，不是反射的機制。（分解量測時同一台機器還有三個代理
  在建置，單組數字有 ±40% 雜訊，所以上面只用三組一致的值互相驗算，而不是拿單一分解下結論；另外「不配置的反射呼叫」
  那一組其實沒有不配置：結果裝箱就在裡面。）
