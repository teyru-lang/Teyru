...
- **標準庫的快取與兩單元建置（W13 第 1、2 項已落地；第 3 項未做，見本條最後）**：一次建置原本把
  ...
  **上面這一條的時間與位元組數是重構當時量的，當時的樹還沒有 W5 的讀取 API；重上 W5
  之後還沒有重量**（重上時機器一直在 load 20 以上，量了也不能用）。重上之後**驗過的是行為**：
  冷快取與暖快取的執行檔對 hello、`examples/webapp`、兩支字串／反射程式在後端 × `-O0`／`-O2`
  的每一格都**逐位元組相同**且輸出、離開碼相同；改 `lib/` 一個字元重編編譯器後，key 變成不同的
  目錄、物件數 1→2、舊物件沒被動到，而新程式印的是新標準庫的字串；把標準庫改回去後原本那一格
  被重用；截斷的物件、翻轉一個位元的物件、沒有 `manifest` 的 entry 都是未命中（`make cache`
  第 8、9 節）。時間與大小要等機器空閒重量。
