...
- **字串的讀取語意是 UTF-16 code unit，儲存是 WTF-8（W5 的第二塊：讀取 API）**：`tystr`
  ...
  `!(out)`；**那一列的長跑量到 0.390 → 0.490 秒（1.26x，9 對交錯，同一支 `ab.sh`），超過
  10% 預算**，而這台機器的負載（進入時 load 17.7、1.9 秒 CPU 的程式量到 128 秒牆鐘）讓 9 對
  不足以把它和雜訊分開。算術上最可疑的是那支 benchmark 的 `charAt` 走訪：取一個 unit 要從
  麵包屑那一格解到 63 個序列，而它是 2000 次存取對 4,000,000 次串接，看不出 +100 毫秒從哪
  來——所以下一步是**安靜的機器上重量**（或把走訪單獨拆出來量），在那之前不要把它當成
  定論，也不要當成沒有回退。若在安靜機器上仍然如此，D1 的備選（compact strings）就有理由
  了。牆鐘在這種負載下沒有意義：同一支 1.9 秒
