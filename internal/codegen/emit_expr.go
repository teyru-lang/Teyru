...
func (e *Emitter) literalDef(name, s string, decl bool) string {
	...
	// initializer says so: an ASCII literal needs no table, and a group for it
	// is one initializer more than the struct has -- which clang accepts as a
	// warning and gcc refuses outright.
	bc := ""
	if l.nbc > 0 {
		bc = ", {0}"
	}
	fmt.Fprintf(&b, "%sstruct %s_s %s = {{&cls_%s, %d, %d, %du}, %s%s};\n", e.link, name, name,
		mangle(e.prog.Builtins.String.Full), len(s), l.units, l.flags, e.cstr(s), bc)
