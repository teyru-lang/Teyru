...
// A cache's failure mode is not a slow build but a wrong one: an artifact that
// is there, is read, and is not the artifact a build wrote. What an entry says
// about its own files is what keeps that from happening, so damage of every
// shape that leaves a file behind is a miss.
func TestEntryRefusesADamagedArtifact(t *testing.T) {
	dir := t.TempDir()
	e := (&Cache{dir: dir}).Lookup(Key{Prelude: "p"})
	object := []byte("the object file, all of it")
	if err := e.Put("prelude.o", object); err != nil {
		t.Fatal(err)
	}
	if !e.Has("prelude.o") {
		t.Fatal("an artifact that was written is not a hit")
	}
	path := e.Path("prelude.o")
	// Truncated: a copy that stopped early, which is what a full disk or an
	// interrupted writer leaves behind.
	if err := os.WriteFile(path, object[:5], 0o644); err != nil {
		t.Fatal(err)
	}
	if e.Has("prelude.o") {
		t.Error("a truncated artifact is a hit")
	}
	// The same length, one byte different: the linker would take this one, which
	// is why the size alone is not the check.
	...
	if e.Has("prelude.o") {
		t.Error("an artifact with a flipped bit is a hit")
	}
	// Rewriting it through Put is what a build that lost its artifact does, and
	// it makes the entry whole again.
	if err := e.Put("prelude.o", object); err != nil {
		t.Fatal(err)
	}
	if !e.Has("prelude.o") {
		t.Error("an artifact that was rewritten is not a hit")
	}
}

// Files without a manifest are a cache somebody else put there -- a directory
// restored from a backup, an older layout, a build that died before it recorded
// what it wrote. Nothing says they are the files a build produced, so nothing
// reads them.
func TestEntryWithoutAManifestIsNotRead(t *testing.T) {
	dir := filepath.Join(t.TempDir(), "prelude", "key")
	if err := os.MkdirAll(dir, 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(dir, "prelude.o"), []byte("object"), 0o644); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(dir, "manifest"), []byte("not a manifest at all\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	e := &Entry{dir: dir}
	if e.Has("prelude.o") {
		t.Error("an entry with no usable manifest is a hit")
	}
}

// An entry holds one build's files together: a manifest that names the header
// and the source but not the object is an entry with a hole, and neither half
// of a build may be taken from it.
func TestEntryNeedsEveryFileItWasAskedFor(t *testing.T) {
	dir := t.TempDir()
	e := (&Cache{dir: dir}).Lookup(Key{Prelude: "p"})
	if err := e.Put("typrelude.h", []byte("header")); err != nil {
		t.Fatal(err)
	}
	if err := e.Put("prelude.c", []byte("source")); err != nil {
		t.Fatal(err)
	}
	if e.Has("typrelude.h", "prelude.c", "prelude.o") {
		t.Error("an entry missing one of the files it was asked for is a hit")
	}
}
